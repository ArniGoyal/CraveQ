...
def selectCoreIngredients(ingredients):
    ...
    if proteins:
        selected.append(proteins[0])
    if grains:
        selected.append(grains[0])
    if vegetables:
        selected.append(vegetables[0])

    # fallback if less than 3 found
    while len(selected) < 3 and len(selected) < len(ingredients):
        for ing in ingredients:
...
def upgradeRecipe(recipe_name):
    ...
    for k, v in validated.items():
        print(f"{k} -> {[item['replacement'] for item in v]}")

    upgraded = generateUpgradedIngredientList(original_ingredients, validated)

    print("\n🥗 Upgraded Ingredient List:")
    print(upgraded)

    print("\n🔎 Searching for healthier recipes...")

    # Get original recipe calories once
    url = f"{BASE_API}/recipe2-api/recipe-bytitle/recipeByTitle?title={recipe_name}"
    response = requests.get(url, headers=HEADERS)
    data = response.json()
    original_calories = float(data["data"][0]["Calories"])
    ...
    for r in recipes:
        ...
        r["HealthScore"] = calculateHealthScore(ingredients)

    recipes.sort(
    key=lambda x: (
        float(x.get("HealthScore", 0)),
        -float(x.get("Calories", 0))
    ),
    reverse=True
)

    # SMART TOP 3 SELECTION

    original_continent = data["data"][0]["Continent"]

    ranked = []

    for r in recipes:
        try:
            recipe_calories = float(r.get("Calories", 0))
            recipe_health = float(r.get("HealthScore", 0))

            health_gain = original_calories - recipe_calories
            score = (health_gain * 2) + recipe_health

            if r.get("Continent") == original_continent:
                score += 10
    ...
    top3 = [r for _, r in ranked[:3]]

    print("\n🏆 TOP 3 HEALTHIEST ALTERNATIVES:\n")

    for i, r in enumerate(top3, 1):

        print(f"\n==============================")
        print(f"🏆 #{i} {r['Recipe_title']}")
        print(f"Calories: {r['Calories']}")
        print(f"HealthScore: {r['HealthScore']}")
        print(f"Region: {r['Region']}")
        print("==============================")

        details = getFullRecipeDetails(r["Recipe_id"])
...
craving = input("Enter your cravings: ")
upgradeRecipe(craving)
